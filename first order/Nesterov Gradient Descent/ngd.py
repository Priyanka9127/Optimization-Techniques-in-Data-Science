# This code implements the Nesterov Accelerated Gradient Descent (NGD) algorithm to optimize the Rosenbrock function.
# The Nesterov Accelerated Gradient Descent optimizer uses momentum and lookahead steps to potentially improve convergence speed.
# The optimization process is visualized using both 2D contour and 3D surface plots, with the optimization path shown in red.
# The algorithm iterates through the optimization steps, and the progress is displayed in real-time via animation.
# The final optimized point is shown in green on both plots.
# The animation includes iteration updates, and you can optionally save the animation as a GIF.
