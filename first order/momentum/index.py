# This code implements Momentum Gradient Descent to optimize the Rosenbrock function.
# The Rosenbrock function is a common test function for optimization algorithms.
# The algorithm is enhanced by using a momentum term to help accelerate convergence.
# The process is visualized using both a 2D contour plot and a 3D surface plot, with an animation showing the optimization path.
# The optimization process is tracked, and the path taken by the algorithm is plotted on both 2D and 3D visualizations.
# The animation shows the iteration process with real-time updates for both the 2D and 3D plots.
