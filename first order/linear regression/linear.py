# This script implements a simple Linear Regression model using gradient descent.
# The steps followed in the code are:
# 1. Define a LinearRegressionModel class that includes methods for:
#    - hypothesis (prediction function): y_hat = X * W + b
#    - loss (Mean Squared Error loss function)
#    - gradient (for gradient descent updates)
#    - fit (train the model using gradient descent)
# 2. Create a 3x3 input feature matrix (X) and a target vector (y).
# 3. Train the model by calling the fit method and track the loss and weights.
# 4. Plot the loss curve over iterations to visualize the training progress.
# 5. Create a 3D plot showing the loss curve and weights' updates during training.
# 6. Make final predictions after training.
# 7. Output the final weights, bias, and predictions.
