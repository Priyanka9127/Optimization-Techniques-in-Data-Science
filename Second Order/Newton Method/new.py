# This script implements Newton's Method for optimization using the Levenberg-Marquardt modification.
# The NewtonMethod class allows optimization of a function using its gradient and Hessian matrix.
# The `optimize` method optimizes the function starting from an initial point and optionally visualizes the optimization path.
# The animation is displayed in both 2D and 3D to show the optimization path taken by the method.
# The Rosenbrock function, commonly used for optimization tests, is used here as the target function.
# The `rosenbrock_grad` and `rosenbrock_hessian` functions compute the gradient and Hessian matrix of the Rosenbrock function.
# The visualization includes a 2D contour plot and a 3D surface plot with the optimization path overlaid.
# The animation is created using Matplotlib’s FuncAnimation for dynamic updating of the plots.
"""
        Initializes the NewtonMethod optimizer with given parameters:
        - alpha: Learning rate for the optimization.
        - damp_factor: Damping factor for the Levenberg-Marquardt method.
        - alpha_optim: Optional parameter for further alpha optimization (not used in current implementation).
        
        Optimizes a function starting from an initial point 'x' using Newton's Method.
        - func_callback: The function to optimize (not used directly here, as we focus on gradient and Hessian).
        - grad_func_callback: The gradient function of the target function.
        - hessian_func_callback: The Hessian function of the target function.
        - is_plot: If True, generates an animation of the optimization path.
        """
...
EPSILON = 0.0001  # Convergence threshold for gradient norm
...
class NewtonMethod:
    ...
    def _reset(self):
        """
        Resets the optimizer’s parameters to their initial values.
        """
        self.alpha = 0.01
